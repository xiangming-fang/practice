 ...
 * 可中断锁测试；
 * ReentrantLock的lockInterruptibly是可中断获取锁方式
 * synchronized是不可中断获取锁方式
 */
public class InterruptLockTest {
    ...
    /**
     * 测试ReentrantLock响应中断
     * 用ReentrantLock中的lockInterruptibly获取锁的方式，可以让线程尝试性中断。
     * 两个线程产生资源竞争，导致死锁，可以手动中断其中一个线程，让其释放资源。
     * 
     * @throws Exception
     */
    @Test
    public void exeInterruptTest() throws Exception {
        InterruptLockTest demo = new InterruptLockTest();
        demo.exeInterrupt();
    }

    /**
     * 测试Synchronized响应中断
     * Synchronized 造成的锁等待线程，是中断不了的
     * 两个线程产生资源竞争，导致死锁，让其中一个线程调用Interrupt方法，中断不了，释放不了锁资源
     * @throws Exception
     */
    @Test
